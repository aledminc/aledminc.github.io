...
export default function ContributionGraph() {
  const { weeks, monthMarkers, total, lastDate } = useMemo(() => {
    ...
    const today = dateAtUtc(isoDate(new Date()))
    ...
    // The fixed anchor is only three days before July. Suppress the older of
    // two labels when month markers would collide in adjacent week columns.
                ...
                <div className="contrib__col" key={week[0]?.date ?? weekIndex}>
                  {Array.from({ length: 7 }, (_, dayIndex) => {
                    ...
                    return day ? (
                      ...
                      <span
                        key={`empty-${dayIndex}`}
                        className="contrib__cell is-empty"
                        aria-hidden="true"
                      />
                    )
                  })}
